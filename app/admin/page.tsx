...
const dashboardLinks = [
  { href: "/admin/customers", title: "Customers", copy: "Search by name, phone or email, edit notes and preferences, and export the customer list." },
  ...
  { href: "/admin/document-vault", title: "Document Vault", copy: "Store private salon records and export complete or category-based business document ZIPs." },
  { href: "/admin/integrations", title: "Integrations", copy: "Check the systems connected to studio operations." },
] as const;

export default function AdminPage() {
  return <main className="admin-dashboard shell"><p className="eyebrow">Studio dashboard</p><h1>Welcome back,<br /><em>Kim.</em></h1><p className="admin-intro">Manage customers, the live service catalog, front-desk queue, private records and the systems that support the salon.</p><div className="admin-card-grid">{dashboardLinks.map((item) => <Link href={item.href} className="admin-card" key={item.href}><h2>{item.title}</h2><p>{item.copy}</p><span>Open <span aria-hidden="true">↗</span></span></Link>)}</div><Link href="/" className="text-link">View public website <span aria-hidden="true">↗</span></Link></main>;
}
